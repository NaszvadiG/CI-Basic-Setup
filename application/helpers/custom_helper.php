...
if (!function_exists('upload_image')) {
    function upload_image($file_name = false, $directory_path = 'image_path')
    {
        $directory_path = realpath('') . "/" . config_item($directory_path);
        $CI = &get_instance();
...
if (!function_exists('resizeImage')) {
    function resizeImage($file_name = false, $directory_path = 'image_path')
    {
        $CI = &get_instance();

        $directory_path = realpath('') . '/' . config_item($directory_path);
        $thumb_directory_path = "{$directory_path}/thumb";
        if (!is_dir($thumb_directory_path)) {
            mkdir($thumb_directory_path, 0755, true);
        }

        $config = [
            'image_library' => 'gd2',
            'source_image' => "{$directory_path}/{$file_name}",
            'source_image' => "{$thumb_directory_path}/{$file_name}",
            'create_thumb' => true,
            'maintain_ratio' => true,
            'width' => 400,
            'height' => 400,
...
if (!function_exists('updateUserSession')) {
    function updateUserSession()
    {
        ...
        $user = $CI->db->get_where('admins', ['id' => $user_id])->row();
        $CI->session->set_userdata('admin', $user);
        return true;
    }
}
...
if (!function_exists('createValidateRules')) {
    function createValidateRules($rulesArr = [], $labelsArr = [])
    {
        $output = [];
        foreach ($rulesArr as $field => $rules) {
            $field_label = str_replace([']', '['], '', $field);

            $output[$field] = [
                'field' => $field,
                'label' => isset($labelsArr[$field_label]) ? $labelsArr[$field_label] : lang($field_label),
                'rules' => $rules,
            ];
        }
        return $output;
